...
@section('content')
    <div class="mx-auto max-w-3xl">
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-extrabold">إضافة لعبة جديدة</h2>
                <p class="text-sm text-[var(--color-text-muted)]">أدخل بيانات اللعبة لحفظها في النادي.</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('games.store') }}"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="6" required
                    class="w-full resize-y rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-4 py-3 text-sm leading-7 text-[var(--color-text)] outline-none transition focus:border-[#D46417]">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-2 text-xs text-red-400">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex flex-col-reverse gap-3 border-t border-[var(--color-border)] pt-5 sm:flex-row sm:justify-end">
                ...
                <button type="submit"
                    class="rounded-xl bg-[#D46417] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#b95714]"><i
                        class="fa-solid fa-check me-2"></i>حفظ اللعبة</button>
            </div>
        </form>
    </div>
@endsection
