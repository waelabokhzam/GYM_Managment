    ...
    <div class="mx-auto max-w-3xl">
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-[var(--color-text-muted)]">حدد وقت الفترة والفئة المسموح لها بالدخول.</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('timeslots.store') }}"
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input id="start_time" name="start_time" type="time" value="{{ old('start_time') }}" required autofocus
            ...
            <div>
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <label
                        ...
                        <input type="radio" name="gender_type" value="mixed" @checked(old('gender_type', 'mixed') === 'mixed')
                            class="accent-[#D46417]">
                        <span><strong class="block text-sm">فترة مختلطة</strong><small
                                class="text-xs text-[var(--color-text-muted)]">مناسبة للجميع</small></span>
                    </label>
                    <label
                        class="flex cursor-pointer items-center gap-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] p-4 transition has-[:checked]:border-[#D46417] has-[:checked]:bg-[#D46417]/10">
                        <input type="radio" name="gender_type" value="women_only"
                            @checked(old('gender_type') === 'women_only') class="accent-[#D46417]">
                        <span><strong class="block text-sm">نساء فقط</strong><small
            ...
            <div class="flex flex-col-reverse gap-3 border-t border-[var(--color-border)] pt-5 sm:flex-row sm:justify-end">
                ...
                <button type="submit"
                    class="rounded-xl bg-[#D46417] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#b95714]"><i
                        class="fa-solid fa-check me-2"></i>حفظ الفترة</button>
            </div>
        </form>
    </div>
@endsection